...
class User:
    def __init__(self, num_reels, interests):
        ...
        self.interaction_matrix = np.zeros(num_reels)
        self.favourite_tags_count = np.zeros(len(interests))
        ...
        if action == 'like':
            ...
            for tag in reel.tags:
                if tag not in self.interests:
                    ...
                    self.favourite_tags_count = np.append(self.favourite_tags_count, 0)
        elif action == 'dislike':
            self.interaction_matrix[reel.id] = -1
        elif action == 'watched':
    ...
    def recommend_ad(self, user):
        ...
        while True:
            ...
            if ad_id not in self.suggested_ads:
                ...
                return next(ad for ad in self.ads if ad.id == ad_id)
        return None
